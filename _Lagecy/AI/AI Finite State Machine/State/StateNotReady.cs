...
public class StateNotReady : AIState
{
    public StateNotReady()
    {
        this.tag = "notReady";
    }
    ...
    public override string CheckTransitions()
    {
        ...
        //if (agent.remainingDistance <= agent.navAgent.stoppingDistance)
        float d =/* agent.remainingDistance < agent.navAgent.stoppingDistance ? */agent.remainingDistance;// : agent.navAgent.stoppingDistance;
        if(d <= agent.navAgent.stoppingDistance+0.1f)
        {
            return "idle";
        }
